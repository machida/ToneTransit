...
> 実装は `js/{presets,summary,quiz,i18n}.js` ほか、`app.js` / `renderer.js` / `fretboard.js` /
> `audio.js` / `index.html` / `css/app.css` に分散。対応するテストは `test/{presets,summary,quiz,i18n}.test.js`
