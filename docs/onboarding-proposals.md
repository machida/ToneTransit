...
> **状況: SPEC-01〜15 はすべて実装済み**（推奨順で対応）。本書は経緯と各機能の仕様の記録として残す。
> 実装は `js/{presets,summary,i18n}.js` ほか、`app.js` / `renderer.js` / `fretboard.js` /
> `audio.js` / `index.html` / `css/app.css` に分散。対応するテストは `test/{presets,summary,i18n}.test.js`
> と既存テストの追加ケース。SPEC-08 の既定は本書草案の beginner ではなく **advanced**（既定の盤面で
> テンションを保つため）に変更している。
> 日常的な利用方法や現行 UI の説明は `README.md` を正とし、本書は実装判断の背景資料として扱う。
...
> この機能は現行 UI から削除済み。以下は実装時の記録として保持する。
